...
const (
	geminiOAuthClientIDEnv     = "CPA_PRO_GEMINI_OAUTH_CLIENT_ID"
	geminiOAuthClientSecretEnv = "CPA_PRO_GEMINI_OAUTH_CLIENT_SECRET"
)

var (
	geminiOAuthClientID     = strings.TrimSpace(os.Getenv(geminiOAuthClientIDEnv))
	geminiOAuthClientSecret = strings.TrimSpace(os.Getenv(geminiOAuthClientSecretEnv))
)
...
const (
	antigravityOAuthClientIDEnv     = "CPA_PRO_ANTIGRAVITY_OAUTH_CLIENT_ID"
	antigravityOAuthClientSecretEnv = "CPA_PRO_ANTIGRAVITY_OAUTH_CLIENT_SECRET"
)

var (
	antigravityOAuthClientID     = strings.TrimSpace(os.Getenv(antigravityOAuthClientIDEnv))
	antigravityOAuthClientSecret = strings.TrimSpace(os.Getenv(antigravityOAuthClientSecretEnv))
)
...
func (h *Handler) refreshGeminiOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	if geminiOAuthClientID == "" || geminiOAuthClientSecret == "" {
		return "", fmt.Errorf("gemini OAuth client credentials are not configured; set %s and %s", geminiOAuthClientIDEnv, geminiOAuthClientSecretEnv)
	}
...
func (h *Handler) refreshAntigravityOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	if antigravityOAuthClientID == "" || antigravityOAuthClientSecret == "" {
		return "", fmt.Errorf("antigravity OAuth client credentials are not configured; set %s and %s", antigravityOAuthClientIDEnv, antigravityOAuthClientSecretEnv)
	}
