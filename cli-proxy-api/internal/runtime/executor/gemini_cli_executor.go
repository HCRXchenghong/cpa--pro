...
var (
	geminiOAuthClientID     = strings.TrimSpace(os.Getenv("CPA_PRO_GEMINI_OAUTH_CLIENT_ID"))
	geminiOAuthClientSecret = strings.TrimSpace(os.Getenv("CPA_PRO_GEMINI_OAUTH_CLIENT_SECRET"))
)
...
func (e *GeminiCLIExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	for idx, attemptModel := range models {
		...
		if httpResp.StatusCode >= 200 && httpResp.StatusCode < 300 {
			...
			var param any
			out := sdktranslator.TranslateNonStream(respCtx, to, from, attemptModel, originalRequestBytes(opts), payload, data, &param)
...
func (e *GeminiCLIExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	for idx, attemptModel := range models {
		...
		go func(resp *http.Response, reqBody []byte, attemptModel string) {
			...
			if opts.Alt == "" {
				...
				for scanner.Scan() {
					...
					if bytes.HasPrefix(line, dataTag) {
						segments := sdktranslator.TranslateStream(respCtx, to, from, attemptModel, originalRequestBytes(opts), reqBody, bytes.Clone(line), &param)
				...
				segments := sdktranslator.TranslateStream(respCtx, to, from, attemptModel, originalRequestBytes(opts), reqBody, []byte("[DONE]"), &param)
				for i := range segments {
					out <- cliproxyexecutor.StreamChunk{Payload: segments[i]}
				}
			...
			reporter.Publish(ctx, helps.ParseGeminiCLIUsage(data))
			var param any
			segments := sdktranslator.TranslateStream(respCtx, to, from, attemptModel, originalRequestBytes(opts), reqBody, data, &param)
			for i := range segments {
				out <- cliproxyexecutor.StreamChunk{Payload: segments[i]}
			}
...
func prepareGeminiCLITokenSource(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth) (oauth2.TokenSource, map[string]any, error) {
	...
	if geminiOAuthClientID == "" || geminiOAuthClientSecret == "" {
		return nil, nil, statusErr{code: http.StatusUnauthorized, msg: "gemini OAuth client credentials are not configured; set CPA_PRO_GEMINI_OAUTH_CLIENT_ID and CPA_PRO_GEMINI_OAUTH_CLIENT_SECRET"}
	}
