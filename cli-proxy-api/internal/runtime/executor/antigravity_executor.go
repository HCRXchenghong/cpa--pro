...
var (
	antigravityClientID               = strings.TrimSpace(os.Getenv("CPA_PRO_ANTIGRAVITY_OAUTH_CLIENT_ID"))
	antigravityClientSecret           = strings.TrimSpace(os.Getenv("CPA_PRO_ANTIGRAVITY_OAUTH_CLIENT_SECRET"))
...
func (e *AntigravityExecutor) refreshToken(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	if antigravityClientID == "" || antigravityClientSecret == "" {
		return auth, statusErr{code: http.StatusUnauthorized, msg: "antigravity OAuth client credentials are not configured; set CPA_PRO_ANTIGRAVITY_OAUTH_CLIENT_ID and CPA_PRO_ANTIGRAVITY_OAUTH_CLIENT_SECRET"}
	}
