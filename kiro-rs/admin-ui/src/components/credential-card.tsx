  ...
  const handleDelete = async () => {
    try {
      if (!credential.disabled) {
        await setDisabled.mutateAsync({ id: credential.id, disabled: true })
      }
      const res = await deleteCredential.mutateAsync(credential.id)
      toast.success(res.message)
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t">
            ...
            <Button
              ...
              disabled={setDisabled.isPending || deleteCredential.isPending}
              title={!credential.disabled ? '将先禁用凭据再删除' : undefined}
            >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {!credential.disabled && ' 当前凭据会先被禁用，然后再删除。'}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button
              ...
              disabled={setDisabled.isPending || deleteCredential.isPending}
            >
              {setDisabled.isPending || deleteCredential.isPending ? '删除中...' : '确认删除'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
