...
import { RefreshCw, LogOut, Moon, Sun, Server, Plus, Upload, FileUp, Trash2, RotateCcw, CheckCircle2, KeyRound, ExternalLink, Loader2, UserPlus, Send } from 'lucide-react'
...
import { useCredentials, useDeleteCredential, useResetFailure, useSetDisabled, useLoadBalancingMode, useSetLoadBalancingMode, useKiroCliOAuthLoginStatus, useStartKiroCliOAuthLogin, useSubmitKiroCliOAuthCallback } from '@/hooks/use-credentials'
...
export function Dashboard({ onLogout }: DashboardProps) {
  ...
  const [startingOAuthAction, setStartingOAuthAction] = useState<'reuse' | 'new' | null>(null)
  ...
  const { mutate: deleteCredential, mutateAsync: deleteCredentialAsync } = useDeleteCredential()
  const { mutate: resetFailure } = useResetFailure()
  const { mutateAsync: setDisabledAsync } = useSetDisabled()
  const { data: loadBalancingData, isLoading: isLoadingMode } = useLoadBalancingMode()
  const { mutate: setLoadBalancingMode, isPending: isSettingMode } = useSetLoadBalancingMode()
  const { data: oauthStatus } = useKiroCliOAuthLoginStatus(true)
  const { mutate: startKiroCliOAuthLogin, isPending: isStartingOAuth } = useStartKiroCliOAuthLogin()
  const { mutate: submitKiroCliOAuthCallback, isPending: isSubmittingOAuthCallback } = useSubmitKiroCliOAuthCallback()
  ...
  const handleBatchDelete = async () => {
    ...
    if (selectedCredentials.length === 0) {
      toast.error('选中的凭据不存在')
      return
    }

    const enabledCount = selectedCredentials.filter(credential => !credential.disabled).length
    const enabledText = enabledCount > 0 ? `其中 ${enabledCount} 个会先禁用再删除。` : ''

    if (!confirm(`确定要删除 ${selectedCredentials.length} 个凭据吗？此操作无法撤销。${enabledText}`)) {
      return
    }
    ...
    for (const credential of selectedCredentials) {
      try {
        if (!credential.disabled) {
          await setDisabledAsync({ id: credential.id, disabled: true })
        }
        await deleteCredentialAsync(credential.id)
        successCount++
      } catch (error) {
        failCount++
      }
  ...
  const handleStartKiroCliOAuth = () => {
    setStartingOAuthAction('reuse')
    startKiroCliOAuthLogin(
      { license: 'free' },
      {
        onSuccess: () => {
          toast.success('已启动官方 Kiro OAuth 登录')
        },
        onError: (error) => {
          toast.error(`启动失败: ${extractErrorMessage(error)}`)
        },
        onSettled: () => {
          setStartingOAuthAction(null)
        }
      }
    )
  }

  const handleAddNewKiroAccount = () => {
    setStartingOAuthAction('new')
    setOauthCallbackUrl('')
    startKiroCliOAuthLogin(
      { license: 'free', forceLogout: true },
      {
        onSuccess: () => {
          toast.success('已退出当前官方 Kiro CLI 账号，开始登录新账号')
        },
        onError: (error) => {
          toast.error(`启动失败: ${extractErrorMessage(error)}`)
        },
  ...
  const handleSubmitOAuthCallback = () => {
    ...
    submitKiroCliOAuthCallback(
      { callbackUrl },
      {
        onSuccess: () => {
          toast.success('OAuth callback 已提交')
          setOauthCallbackUrl('')
        },
        onError: (error) => {
  ...
  useEffect(() => {
    if (oauthStatus?.phase === 'completed' && oauthStatus.importedCredentialId) {
      toast.success(`OAuth 登录完成，凭据 #${oauthStatus.importedCredentialId} 已导入`)
      queryClient.invalidateQueries({ queryKey: ['credentials'] })
    } else if (oauthStatus?.phase === 'failed' && oauthStatus.error) {
      toast.error(oauthStatus.error)
    }
  }, [oauthStatus?.phase, oauthStatus?.importedCredentialId, oauthStatus?.error, queryClient])
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 md:px-8 py-6">
        ...
        <div className="space-y-4">
          {oauthStatus && (oauthStatus.running || oauthStatus.phase !== 'idle') && (
            <Card>
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 font-medium">
                      ...
                      ) : oauthStatus.phase === 'completed' ? (
                        <CheckCircle2 className="h-4 w-4 text-green-600" />
                      ) : (
                        <KeyRound className="h-4 w-4" />
                      )}
                      官方 Kiro OAuth
                      <Badge variant={oauthStatus.phase === 'completed' ? 'success' : oauthStatus.phase === 'failed' ? 'destructive' : 'secondary'}>
                        {oauthStatus.phase}
                      </Badge>
                    </div>
                    {oauthStatus.loginUrl && (
                      <a
                  ...
                  {oauthStatus.output.length > 0 && (
                    <pre className="max-h-36 w-full overflow-auto rounded border bg-muted p-3 text-xs md:max-w-xl">
                      {oauthStatus.output.slice(-12).join('\n')}
                    </pre>
                  )}
                </div>
              </CardContent>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap justify-start gap-2 lg:justify-end">
              {selectedIds.size > 0 && (
                <>
                  ...
                  <Button
                    ...
                    title={selectedDisabledCount < selectedIds.size ? '未禁用的凭据会先禁用再删除' : undefined}
                  >
              ...
              <Button
                onClick={handleStartKiroCliOAuth}
                size="sm"
                variant="outline"
                disabled={isStartingOAuth || oauthStatus?.running}
              >
                {startingOAuthAction === 'reuse' ? (
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                ) : (
              ...
              <Button
                onClick={handleAddNewKiroAccount}
                size="sm"
                variant="secondary"
                disabled={isStartingOAuth || oauthStatus?.running}
                title="退出官方 Kiro CLI 当前账号，再登录一个新的 Kiro 账号"
              >
                {startingOAuthAction === 'new' ? (
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                ) : (
