...
fn apply_usage_metadata(
    ...
    if let Some(subscription_title) = usage_limits.subscription_title() {
        if credentials.subscription_title.as_deref() != Some(subscription_title) {
            credentials.subscription_title = Some(subscription_title.to_string());
            changed = true;
        }
    }

    if let Some(email) = usage_limits
        .email()
        .filter(|email| !email.trim().is_empty())
    {
        if credentials.email.as_deref() != Some(email) {
            credentials.email = Some(email.to_string());
            changed = true;
        }
    }

    if let Some(user_id) = usage_limits
        .user_id()
        .filter(|user_id| !user_id.trim().is_empty())
    {
        let account_id_hash = sha256_hex(user_id);
        if credentials.account_id_hash.as_deref() != Some(account_id_hash.as_str()) {
            credentials.account_id_hash = Some(account_id_hash);
            changed = true;
        }
    }
...
impl MultiTokenManager {
        ...
        ManagerSnapshot {
            entries: entries
                ...
                .map(|e| CredentialEntrySnapshot {
                    ...
                    account_id_hash: e.credentials.account_id_hash.clone(),
                    success_count: e.success_count,
                    last_used_at: e.last_used_at.clone(),
    ...
    pub async fn get_usage_limits_for(&self, id: u64) -> anyhow::Result<UsageLimitsResponse> {
        ...
        // 更新账号元数据到凭据（订阅等级、邮箱、稳定账号哈希）
        let changed = {
            let mut entries = self.entries.lock();
            entries
                .iter_mut()
                .find(|e| e.id == id)
                .map(|entry| apply_usage_metadata(&mut entry.credentials, &usage_limits))
                .unwrap_or(false)
        };

        if changed {
            if let Err(e) = self.persist_credentials() {
                tracing::warn!("账号元数据更新后持久化失败（不影响本次请求）: {}", e);
            }
        }
    ...
    pub async fn add_credential(&self, new_cred: KiroCredentials) -> anyhow::Result<u64> {
        ...
        validated_cred.auth_region = new_cred.auth_region.clone();
        validated_cred.api_region = new_cred.api_region.clone();
        validated_cred.machine_id = new_cred.machine_id.clone();
        validated_cred.email = new_cred.email.clone();
        validated_cred.account_id_hash = new_cred.account_id_hash.clone();
        validated_cred.subscription_title = new_cred.subscription_title.clone();
        ...
        validated_cred.endpoint = new_cred.endpoint.clone();
        ...
        if !validated_cred.is_api_key_credential() {
            if let Some(token) = validated_cred.access_token.clone() {
                ...
                {
                    Ok(usage_limits) => {
                        apply_usage_metadata(&mut validated_cred, &usage_limits);
                    }
                    Err(err) => {
                        tracing::warn!("新增凭据时获取账号元数据失败，将跳过稳定账号去重: {}", err);
                    }
                }
            }
        }

        // 5. 基于稳定账号身份检测重复；refreshToken 轮换时仍更新原凭据而不新增。
        let identity_duplicate_id = {
            let account_id_hash = validated_cred.account_id_hash.as_deref();
            let profile_arn = validated_cred.profile_arn.as_deref();
            if account_id_hash.is_none() && profile_arn.is_none() {
                None
            } else {
                let endpoint = validated_cred
                    .endpoint
                    .as_deref()
                ...
                entries
                    ...
                    .find(|entry| {
                        let entry_endpoint = entry
                            ...
                            .as_deref()
                            .unwrap_or(&self.config.default_endpoint);
                        if entry_endpoint != endpoint {
                            return false;
                        }

                        (account_id_hash.is_some()
                            && entry.credentials.account_id_hash.as_deref() == account_id_hash)
                            || (profile_arn.is_some()
                                && entry.credentials.profile_arn.as_deref() == profile_arn)
                    })
                    .map(|entry| entry.id)
            }
        };

        if let Some(existing_id) = identity_duplicate_id {
            let updated = {
                let mut entries = self.entries.lock();
                if let Some(entry) = entries.iter_mut().find(|entry| entry.id == existing_id) {
                    let keep_manual_disabled =
                        entry.disabled_reason == Some(DisabledReason::Manual);
                    let existing_cred = entry.credentials.clone();
                    validated_cred.id = Some(existing_id);
                    validated_cred.priority = existing_cred.priority;
                    validated_cred.disabled = keep_manual_disabled;
                    if new_cred.machine_id.is_none() {
                        validated_cred.machine_id = existing_cred
                            .machine_id
                            .or_else(|| validated_cred.machine_id.clone());
                    }
                    if new_cred.region.is_none() {
                        validated_cred.region = existing_cred
                            .region
                            .or_else(|| validated_cred.region.clone());
                    }
                    if new_cred.auth_region.is_none() {
                        validated_cred.auth_region = existing_cred
                            .auth_region
                            .or_else(|| validated_cred.auth_region.clone());
                    }
                    if new_cred.api_region.is_none() {
                        validated_cred.api_region = existing_cred
                            .api_region
                            .or_else(|| validated_cred.api_region.clone());
                    }
                    if new_cred.proxy_url.is_none() {
                        validated_cred.proxy_url = existing_cred
                            .proxy_url
                            .or_else(|| validated_cred.proxy_url.clone());
                        validated_cred.proxy_username = existing_cred
                            .proxy_username
                            .or_else(|| validated_cred.proxy_username.clone());
                        validated_cred.proxy_password = existing_cred
                            .proxy_password
                            .or_else(|| validated_cred.proxy_password.clone());
                    }
                    if validated_cred.email.is_none() {
                        validated_cred.email = existing_cred.email;
                    }
                    if validated_cred.subscription_title.is_none() {
                        validated_cred.subscription_title = existing_cred.subscription_title;
                    }
                    entry.credentials = validated_cred.clone();
                    if keep_manual_disabled {
                        entry.disabled = true;
                        entry.disabled_reason = Some(DisabledReason::Manual);
                    } else {
                        entry.disabled = false;
            ...
            if updated {
                self.persist_credentials()?;
                tracing::info!("凭据 #{} 已存在，已用最新 OAuth 凭据更新", existing_id);
