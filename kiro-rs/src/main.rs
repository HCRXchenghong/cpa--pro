...
use clap::Parser;
use kiro::credential_import::{ImportKiroCliOptions, import_kiro_cli_credentials};
    ...
    if let Some(command) = args.command {
        match command {
            Command::ImportKiroCli {
                ...
                match import_kiro_cli_credentials(ImportKiroCliOptions {
                    db_path: db,
                    credentials_path,
                    replace,
                    priority,
                    ...
                    Ok(result) => {
                        tracing::info!(
                            db = %result.db_path.display(),
                            credentials = %result.credentials_path.display(),
                    ...
                    Err(e) => {
                        tracing::error!("Kiro CLI 凭据导入失败: {}", e);
                        std::process::exit(1);
                    }
                }
                return;
