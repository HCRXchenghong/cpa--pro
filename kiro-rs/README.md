...
如果已经安装并登录官方 Kiro CLI，可直接从 SQLite 登录库导入：

```bash
kiro-cli login
./target/release/kiro-rs --credentials credentials.json import-kiro-cli
```

默认读取 macOS 的 `~/Library/Application Support/kiro-cli/data.sqlite3`、
Linux 的 `~/.local/share/kiro-cli/data.sqlite3`，也兼容 `~/.local/share/amazon-q/data.sqlite3`。如果路径不同：

```bash
./target/release/kiro-rs --credentials credentials.json import-kiro-cli --db /path/to/data.sqlite3
```
